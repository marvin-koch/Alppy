...
```
  POST /adaptive/propose  {class | students, subject, n_groups, generate?}
        │
        ▼
  services/adaptive_service.py::propose_adaptive
        │
        ├─ latest_snapshots(students)     only the LATEST snapshot per competency
        │
        ├─ n_groups == 1 ─────▶ _plan_for_group(whole class)
        ├─ 1 < n < size  ─────▶ cluster_students ──▶ _plan_for_groups → _plan_for_group ×N
        ...
        the teacher: approve · discard · regenerate · move a student between groups
                    ▼
  POST /adaptive/batch → Job(GENERATE_ADAPTIVE)
        │
